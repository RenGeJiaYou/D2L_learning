...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    net.apply(init_weights)  # 为 net 添加权重
    print('training on', device)
    net.to(device)  # 转至 GPU
    ...
    for epoch in range(num_epochs):
        ...
        t_b = time.time()
        print(f"epoch {i} 耗时 {(t_b - t_a) // 60} 分 {(t_b - t_a) % 60:.2f} 秒")
        t_a = t_b
    ...
    print(f'{metric[2] * num_epochs / timer.sum():.1f} examples/sec '
          f'on {str(device)}')
    t_end = time.time()
    print(f"总耗时 {(t_end - t_start) // 60} 分 {(t_end - t_start) % 60:.2f} 秒")
